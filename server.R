...
setwd("C:\\Users\\user\\Documents\\Documents")
library(shiny)
...
shinyServer(function(input, output, session) {
  consumer_api_key <-
    "wwDYtJuVMjSx9T7KiFc5shlNO" # Enter your consumer key
  consumer_api_secret <-
    "SqawhHEAtYPcJmmxM9jRoaf1xEQZYmnkqJsTOHu4PrXwg2C4I8" # Enter your consumer secret
  access_token <-
    "1162651694892634112-oxpriH7pPialPPSkgtSS2nX5u6UB0t" # Enter your access token
  access_token_secret <-
    "0gkXxlYXQqJQ8JmR7bL87hvBl2KDYLckYA7B9xv4spOLF" # Enter your access secret
  ...
  pos.words = scan('positive_words2.txt',
                   what = 'character',sep = ',', quote = "") #Provide your path to "positive_words.txt" file
  neg.words = scan('negative_words2.txt',
                   what = 'character',sep = ',', quote = "") #Provide your path to "negative_words.txt" file
  ...
  score.sentiment <-
             ...
             .progress = 'none') #by default
    {
      ...
      list = lapply(sentences, function(sentence, pos.words, neg.words)
      {
        ...
        for (key in pos.words){
          if (str_detect(sentence, key)){
            pos.matches = pos.matches + 1;
          }
        }
        ...
        for (key in neg.words){
          if (str_detect(sentence, key)){
            neg.matches = neg.matches + 1;
          }
        }
  ...
  output$plot <- renderPlot({
    ...
    pos <-
      scan('positive_words2.txt',
           what = 'character',sep = ',', quote = "") #folder with positive dictionary
    neg <-
      scan('negative_words2.txt',
           what = 'character',sep = ',', quote = "") #folder with negative dictionary
